...
export default function AddTutorial({isDark}) {
    ...
    //exercises adding
    const [exercises, setExercises] = useState([{question: '', answering_section: '', answer: ''}])
    ...
    const onsubmit = () => {
        ...
        axios.post("http://localhost:5000/admin/tutorials/addNewTutorial", data).then((res)=>{
            if(res.data.success){
                ...
                setExercises([{ question: '', answering_section: '', answer: '' }])
                message.success("Successfully Added")
            }
        }).catch((error)=>{
            console.error(error)
            message.error(`Adding failed, ${error}`)
        })
    }
